...
<div data-aeris-catalog-ui-dropdown>
  <ul ref="list" :style='listStyle' @wheel='scrollList'>
    <li v-for="item in list" @click="selectValue($event, item)" :class="{'active': item == value}">{{item}}</li>
  </ul>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    scrollList(e) {
      e.stopPropagation();
      e.deltaY < 0 && this.$refs.list.scrollTop == 0 || e.deltaY > 0 && (this.$refs.list.scrollTop + this.$refs.list.clientHeight) == this.$refs.list.scrollHeight ? e.preventDefault() : null;
    },
