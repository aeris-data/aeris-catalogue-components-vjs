    ...
    <div data-aeris-tree-checkbox-layout>
        ...
        <main v-for="(item,index) of items" :key="item.name">
            <div class="first-level">
                <main>
                    <input type="checkbox" :id="`${name}${item.name}`" :checked="item.checked"
                           @change="checkFirstLevel(index)">
                    <label :for="`${name}${item.name}`">{{item.label}}</label>
                </main>
                <aside>
                    <i v-if="item.downloadable" class="fa fa-shopping-cart tooltip downloadable"
                       :title="$t('directlydownloadable')"></i>
                    <span v-if="item.subitems.length > 0" class="badge">{{item.subitems.length}}</span>
                    <i v-if="item.subitems.length > 0" v-show="item.deployed" class="fa fa-minus-square-o deployed"
                       @click="toggle(index)"></i>
                    <i v-if="item.subitems.length > 0" v-show="!item.deployed" class="fa fa-plus-square-o deployed"
                       @click="toggle(index)"></i>
                </aside>
            </div>
