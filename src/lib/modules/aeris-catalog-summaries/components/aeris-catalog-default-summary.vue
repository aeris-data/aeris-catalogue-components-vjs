...
<template>
  <div :class="{ showBody: deployed }" data-template="summary" @click="displayDetails">
    <div v-if="downloadable" class="cartButton">
      <i
        v-if="!isInCart"
        key="Add"
        :title="$t('addingToCart')"
        class="cartouche fa fa-download addToCartButton"
      ...
      <i
        ...
        class="fa fa-check removeToCartButton"
        @click.stop="removeCartItem(collectionId)"
      />
    </div>
    <main>
    ...
    <footer>
      <div v-for="project in projectList" v-if="projectList" :key="project.projectName" class="cartouche">
        {{ project.projectName }}
      </div>
    </footer>
  </div>
    ...
    handleSuccess: function(response) {
      ...
      if (downloadEntries.length > 0) {
        ...
        document.dispatchEvent(
          new CustomEvent("aerisLongActionStopEvent", { detail: { message: this.$t("addingToCart") } })
        );
      }
    },
