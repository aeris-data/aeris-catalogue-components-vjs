    ...
    <main v-for="(item, index) of items" :key="item.search">
      <div class="first-level">
        ...
        <aside>
          ...
          <i
            v-if="thesaurusItemsLength(item) > 0"
      ...
      <div v-if="item.deployed">
        <div
          v-for="(thesaurusItem, indexSubitem) of item.thesaurusItems"
          :key="thesaurusItem.search"
          class="second-level"
        >
          <div>
            <main>
              <input
                :id="`${name}${item.name}${thesaurusItem.name}`"
                :checked="thesaurusItem.checked"
                type="checkbox"
            ...
            <aside>
              <span v-if="thirdLevelLength(thesaurusItem) > 0" class="badge">{{
                thirdLevelLength(thesaurusItem)
              }}</span>
              <i
                v-if="thirdLevelLength(thesaurusItem) > 0"
                v-show="thesaurusItem.deployed"
                class="fa fa-minus-square-o deployed"
                @click="toggleThirdLevel(index, indexSubitem)"
              />
              <i
                v-if="thirdLevelLength(thesaurusItem) > 0"
                v-show="!thesaurusItem.deployed"
          ...
          <div v-show="thesaurusItem.deployed" class="third-level">
            <div
              v-for="(thirdthesaurusItem, indexThirdthesaurusItem) of thesaurusItem.thesaurusItems"
              :key="thirdthesaurusItem.search"
            >
              <input
                :id="`${name}${item.name}${thesaurusItem.name}${thirdthesaurusItem.name}`"
                :checked="thirdthesaurusItem.checked"
                type="checkbox"
                @change="checkThirdLevel(index, indexSubitem, indexThirdthesaurusItem)"
              />
              <label :for="`${name}${item.name}${thesaurusItem.name}${thirdthesaurusItem.name}`">{{
                thirdthesaurusItem.label
              }}</label>
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    checkSecondLevel(index, indexSubitem, aboveLevel) {
      if (typeof aboveLevel != "undefined") {
        ...
        this.items[index].thesaurusItems[indexSubitem].checked = !this.items[index].thesaurusItems[indexSubitem]
          .checked;
      }
      ...
      if (this.items[index].thesaurusItems[indexSubitem].checked) {
        let searchValues = searchString.substring(
          0,
          searchString.lastIndexOf(";") == -1 ? searchString.length : searchString.lastIndexOf(";")
        );
    ...
    checkThirdLevel(index, indexSubitem, indexThirdthesaurusItem, aboveLevel) {
      if (typeof aboveLevel != "undefined") {
        ...
        this.items[index].thesaurusItems[indexSubitem].thesaurusItems[indexThirdthesaurusItem].checked = !this.items[
          index
        ].thesaurusItems[indexSubitem].thesaurusItems[indexThirdthesaurusItem].checked;
      }
    ...
    toggleThirdLevel(index, indexSubitem) {
      this.items[index].thesaurusItems[indexSubitem].deployed = !this.items[index].thesaurusItems[indexSubitem]
        .deployed;
      this.$nextTick(function() {
        // Defer the callback to be executed after the next DOM update cycle
        // otherwise badges won't be visible on first load
      ...
      let itemsArray = response.body
        .map(item => {
          return {
            ...
            thesaurusItems: item.thesaurusItems
              .map(subitem => {
                return {
                  checked: false,
                  deployed: false,
                  ...
                  label:
                    this.lang == "fr"
                      ? subitem.translation.fr
                        ? subitem.translation.fr
                        : subitem.translation.en
                      : subitem.translation.en,
                  search: subitem.searchConcat,
                  thesaurusItems: subitem.thesaurusItems
                    .map(subsubitem => {
                      return {
                        checked: false,
                        deployed: false,
                        ...
                        label:
                          this.lang == "fr"
                            ? subsubitem.translation.fr
                              ? subsubitem.translation.fr
                              : subsubitem.translation.en
                            : subsubitem.translation.en,
